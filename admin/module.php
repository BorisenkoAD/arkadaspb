...
public $dbh; /*экземпляр соединения с БД*/
private $username = 'u1063074_losta';
private $password = 'zZ3dpH;>t_tWgfeK';
 
function __construct($username,$password){
	$this -> dbh = new PDO('mysql:host=192.168.137.106;dbname=db1063074_losta', $username, $password, array(PDO::ATTR_PERSISTENT => true));
					$this -> dbh -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
					$this -> dbh ->prepare("SET NAMES 'utf8'");
					$this -> dbh ->execute();
 }
...
/* 	function reg() {
		...
		if ($this->check_new_user($login, $passwd, $passwd2, $mail)) {
			...
			$user_key = $this->generateCode(10);
			$passwd = md5($user_key.$passwd.SECRET_KEY); //~ password hash with the private key and user key					
			$this -> dbh ->prepare("INSERT INTO users (login_user, passwd_user, mail_user, sex_user, key_user) 
										VALUES 
										(:login,
										 :passwd,
										 :mail,
			...
			$this -> dbh ->bindParam(':login', $login);
			$this -> dbh ->bindParam(':passwd', $passwd);
			$this -> dbh ->bindParam(':passwd2', $passwd2);
			$this -> dbh ->bindParam(':mail', $mail);
			$this -> dbh ->bindParam(':sex', $sex);
			$this -> dbh ->bindParam(':user_key', $user_key);	
			$stms = $this -> dbh ->execute();			
			if ($stmt) {
				return true;
			} else {
				self::$error='An error occurred while registering a new user. Contact the Administration.';
